...
class elFinder {
    ...
    private $_options = array(
        ...
        'mimeDetect' => 'gd', // files mimetypes detection method (finfo, mime_content_type, linux (file -ib), bsd (file -Ib), internal (by extensions))
    ...
    private function _tmb($img, $tmb) {
        ...
        switch ($this->_options['imgLib']) {
            case 'imagick':
                ...
                return $_img->ThumbnailImage($tmbSize, $tmbSize) && $_img->writeImage($tmb);
                break;
            ...
            case 'gd':
                ...
                list($x, $y, $size) = $this->_cropPos($s[0], $s[1]);
                $oldWidth = imagesx($_img);
                $oldHeight = imagesy($_img);
                if (!imagecopyresampled($_tmb, $_img, 0, 0, 0, 0, $tmbSize, $tmbSize, $oldWidth, $oldHeight)) {
                    return false;
                }
